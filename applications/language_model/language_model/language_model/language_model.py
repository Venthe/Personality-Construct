...
class LanguageModel:
    def __init__(self, model_path=None, device_map = "cpu"):
        ...
        self._tokenizer = AutoTokenizer.from_pretrained(pretrained_model_name_or_path = model_path, device_map=device_map)
        ...
        self._model = AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path = model_path, device_map=device_map, attn_implementation="eager")
        logger.debug(f"Model started in {(time.time()-model_start):0.3f}s, {self._model.device}")
    ...
    def generate(
        ...
        #     decoded_word = self.__decode_single_word(
        #         outputs, first_word=i == 0
        #     )
